...
class MyAI {
 ...
 private:
  ...
  unordered_map<string, int> transposition[2];
  int killer_heuristic[2][MAX_DEPTH][10];
  int killer_heuristic_top[2][MAX_DEPTH];
  ...
  bool Referee(const int* board, const int Startoint, const int EndPoint,
               const int color);
  int Expand(const ChessBoard board, const int color, int* Result, int depth);
  double Evaluate(const ChessBoard* chessboard, const int legal_move_count,
                  const int color);
  double alphaBeta(ChessBoard chessboard, int* move, const int color,
                   const int depth, const int remain_depth, double alpha,
                   double beta);
  bool isDraw(const ChessBoard* chessboard);
  bool toBeEaten(int x, int y);
  bool quiet(const ChessBoard* chessboard, int color);
  double quiescenceSearch(const ChessBoard chessboard, int* move,
                          const int color, const int depth,
                          const int remain_depth, double alpha, double beta);
  bool isThreateningMove(const ChessBoard* chessboard, int move, int color);
  string boardToString(const int* board);
